...
if($situacao == 1){
	...
	$resultado_prod_dest=mysqli_query($conectar,"SELECT * FROM destaques_produtos WHERE nivel_dois='1' ORDER BY ordem ASC");
	...
	while($linhas_prod_dest = mysqli_fetch_array($resultado_prod_dest)){
		...
		if($contr_post_alt == 1){
			...
			$query = mysqli_query($conectar,"UPDATE destaques_produtos set ordem = '$ord_prod_org', modified = NOW() WHERE id=$produto_dest_id");
			$query = mysqli_query($conectar,"UPDATE destaques_produtos set ordem = '$produto_dest_ord_post', modified = NOW() WHERE id=$id");
			$contr_post_alt = 0;
		}
		
		//alterar para 9999 o produto que deseja descer a posição
		if($id === $linhas_prod_dest['id']){
			$query = mysqli_query($conectar,"UPDATE destaques_produtos set ordem = '9999', modified = NOW() WHERE id='$id'");
...
if($situacao == 2){
	//Pesquisar o id dos produtos em destaque no nivel 1 página principal
	$resultado_prod_dest=mysqli_query($conectar,"SELECT * FROM destaques_produtos WHERE nivel_dois='1' ORDER BY ordem DESC");
